...
INPUT_FILE = "processed/final_response_evaluation_20.csv"
OUTPUT_FILE = "processed/final_response_judge_results_20.csv"
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are a strict evidence-based evaluation judge. "
                    "Return only valid JSON."
                ),
            },
            {
...
def main():
    ...
    # Resume from an existing partial result file if one exists.
    if os.path.exists(OUTPUT_FILE):
        existing_df = pd.read_csv(OUTPUT_FILE)

        completed_ids = set(
            existing_df["eval_id"].astype(str).tolist()
        )

        results = existing_df.to_dict("records")

        print(
            f"Existing judged responses found: {len(existing_df)}"
        )
        print(
            f"Remaining responses: {len(df) - len(completed_ids)}"
        )
    else:
        completed_ids = set()
        results = []
    ...
    for index, row in df.iterrows():
        ...
        if eval_id in completed_ids:
            print(
                f"Skipping response {index + 1}/{len(df)} "
                f"(already judged)"
            )
            continue

        print(
            f"Judging response {index + 1}/{len(df)}"
        )
        ...
        try:
            ...
            print()
            print(
                "Partial results have already been saved."
            )
            print(
            ...
            print()
            print(f"Judge failed for eval_id={eval_id}")
            print(f"Reason: {exc}")
            print(
                "Stopping so the failed example can be retried "
                "without corrupting the evaluation."
            )
            break
        ...
        print(
            f"Saved {len(results)}/{len(df)} judgments."
        )

        # Small delay between requests.
        time.sleep(1)

    output_df = save_results(results)
